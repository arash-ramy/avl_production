...
class DeviceMonthlyDistanceCron {
  static async setDeviceDistance() {
    try {
      ...
      fixLastLocation();
      const vehicles = await VehicleModel.find({}).select("_id deviceIMEI");
      if (!vehicles) {
        ...
        vehicles.map(async (vehicle) => {
          ...
          const vehicleLocations = locations.map(({ lat, lng }) => ({
            latitude: lat,
            longitude: lng,
          }));
          vehicle.currentMonthDistance = (
            geolib.getPathLength(vehicleLocations) / 1000.0
          ).toFixed(2);
          vehicle.save();
        });
      }
